...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ product, isOpen, onClose }) => {
  const { addToCart, products, language } = useApp();
  ...
  const currentVariant = current.variants?.find(v => v.size === selectedSize) || current.variants?.[0];
  const availableStock = currentVariant ? currentVariant.stock : 0;
  const unitPrice = currentVariant ? currentVariant.price : 0;
  const totalPrice = unitPrice * quantity;

  const moreLikeIt = getRecommendations(current, products, 4);
  const tName = current.translations?.[language]?.name || current.name;
  const tDesc = current.translations?.[language]?.description || current.description;

  const handleAddToCart = () => {
    if (!currentVariant) return;
    for (let i = 0; i < quantity; i++) {
      addToCart(current, currentVariant.size);
    }
    setJustAdded(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.1fr]">
          <div className="relative aspect-[4/5] md:aspect-auto md:min-h-[560px] overflow-hidden bg-[#f5f3ef] select-none">
            ...
            <div className="absolute bottom-4 left-4 flex items-center gap-1.5 bg-white/90 backdrop-blur-sm px-3 py-1.5">
              ...
              <span className="text-[11px] font-black text-black">{current.rating?.toFixed(1) || "5.0"}</span>
              <span className="text-[9px] text-black/45 font-medium">({current.reviewsCount || 0} avis)</span>
            </div>
          </div>

          <div className="flex flex-col p-7 sm:p-10 lg:p-12 bg-white min-h-0" dir={language === "ar" ? "rtl" : "ltr"}>
              ...
              <div className="flex gap-2 flex-wrap">
                {current.variants?.map(variant => (
                  <button
                    ...
                    className={`flex-1 min-w-[100px] py-3 border text-xs font-bold transition-all duration-200 ${
        ...
        {/* Reviews Section */}
        <div className="px-7 sm:px-10 lg:px-12 pb-12" dir={language === "ar" ? "rtl" : "ltr"}>
          <ReviewSection productId={current.id} />
        </div>

        {moreLikeIt.length > 0 && (
          <div className="border-t border-black/6 bg-[#faf9f6]" dir={language === "ar" ? "rtl" : "ltr"}>
            <div className="px-7 sm:px-10 lg:px-12 pt-8 pb-6 flex items-center justify-between">
              ...
              <ChevronRight className={`h-5 w-5 text-black/15 ${language === "ar" ? "rotate-180" : ""}`} />
            </div>

            <div className="px-7 sm:px-10 lg:px-12 pb-10 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              {moreLikeIt.map(related => {
                const rStock = related.variants?.reduce((sum, v) => sum + v.stock, 0) ?? 0;
                const isOut = rStock === 0;
                const hasDiscount = (related.discountPercent ?? 0) > 0;
                const rPrice = related.variants?.[0]?.price || 0;
                const displayPrice = hasDiscount
                  ? rPrice * (1 - (related.discountPercent ?? 0) / 100)
                  : rPrice;
                const relName = related.translations?.[language]?.name || related.name;
