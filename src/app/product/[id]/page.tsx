...
const formatDZD = (usd: number) =>
  Math.round(usd * USD_TO_DZD).toLocaleString("fr-DZ") + " DA";
...
export default function ProductDetailPage({
  ...
  const price100 = discountedPrice * 1.45;

  const totalStock = (product.stock?.[selectedSize] || 0);
          ...
          <div className="space-y-6 lg:pt-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="text-[8px] font-bold uppercase tracking-[0.25em] text-black/30 border border-black/10 px-2.5 py-1">
                  {product.category}
                </span>
                {product.brand && (
              ...
              <h1 className="font-serif text-3xl sm:text-4xl font-light tracking-wide text-black leading-tight">
                {product.name}
              </h1>
            ...
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-black/30 font-medium">Prix pour 50ml · Tous les flacons sont authentiques et scellés</p>
            </div>

            {/* Size selector */}
            <div className="space-y-2.5">
              <label className="text-[8px] font-bold uppercase tracking-[0.25em] text-black/40">
                Taille du flacon
              </label>
                ...
                {(["50ml", "100ml"] as const).map((size) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={stock === 0}
                    >
                      <span className="text-xs font-bold tracking-wide">{size}</span>
                      <span className="text-[8px] opacity-60">
                        {size === "100ml" ? formatDZD(price100) : formatDZD(discountedPrice)}
                      </span>
                      {stock > 0 && stock <= 5 && (
            ...
            <div className="flex gap-3">
              <div className="flex items-center border border-black/10">
                <button
                  ...
                  className="w-10 h-12 flex items-center justify-center text-black/40 hover:text-black hover:bg-black/5 transition-all"
                >
              ...
              <button
                className="w-12 h-12 flex items-center justify-center border border-black/10 hover:border-black/30 text-black/30 hover:text-black transition-all"
            ...
            {/* Trust badges */}
            <div className="grid grid-cols-3 gap-3 py-4 border-y border-black/5">
              {[
                { icon: Truck, label: "Livraison rapide", sub: "Wilaya & domicile" },
                { icon: Shield, label: "100% Authentique", sub: "Flacons scellés" },
                { icon: RotateCcw, label: "Retour 7 jours", sub: "Satisfait ou remboursé" },
              ].map(({ icon: Icon, label, sub }) => (
                <div key={label} className="flex flex-col items-center text-center gap-1.5">
                  <Icon className="h-4 w-4 text-black/30" />
                  <span className="text-[8px] font-bold uppercase tracking-[0.1em] text-black/50">{label}</span>
                  <span className="text-[7px] text-black/25">{sub}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              <h2 className="text-[9px] font-bold uppercase tracking-[0.25em] text-black/30">Description</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 sm:gap-6">
              {relatedProducts.map((rel) => {
                const relDiscount = (rel.discountPercent ?? 0) > 0
                  ? rel.price * (1 - (rel.discountPercent ?? 0) / 100)
                  : rel.price;
