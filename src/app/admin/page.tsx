...
export default function AdminDashboard() {
  ...
  const getStockStatus = (product: Product) => {
    const totalStock = product.stock["50ml"] + product.stock["100ml"];
    const threshold = product.lowStockAlert || 5;
    
    if (totalStock === 0) return "out-of-stock";
    if (totalStock <= threshold || product.stock["50ml"] <= threshold || product.stock["100ml"] <= threshold) {
      return "low-stock";
    }
    return "in-stock";
  };
  ...
  const filteredStockProducts = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      const totalA = a.stock["50ml"] + a.stock["100ml"];
      const totalB = b.stock["50ml"] + b.stock["100ml"];
  ...
  const stockStats = useMemo(() => {
    ...
    const totalUnits = products.reduce((sum, p) => sum + p.stock["50ml"] + p.stock["100ml"], 0);
    return { total, inStock, lowStock, outOfStock, totalUnits };
  }, [products]);

  const startStockEditing = (product: Product) => {
    setEditingStockId(product.id);
    setEditStockValues({ "50ml": product.stock["50ml"], "100ml": product.stock["100ml"] });
  };

  const saveStockEditing = (productId: string) => {
    updateProductStock(productId, "50ml", editStockValues["50ml"]);
    updateProductStock(productId, "100ml", editStockValues["100ml"]);
        ...
        >
          ...
          <div className="p-10">
            ...
            <div className="text-center mb-10">
              <div className="inline-flex items-center gap-3 mb-4">
                ...
                <div className="text-left">
                  <h1 className="text-xl font-bold tracking-[0.15em] text-neutral-900 uppercase">Perfum Guy</h1>
                  <p className="text-xs tracking-[0.3em] text-neutral-400 uppercase font-medium">Administration</p>
                </div>
              </div>
            ...
            <button
              ...
              disabled={adminSubmitting}
              className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-neutral-200 hover:border-neutral-400 bg-neutral-50 hover:bg-neutral-100 text-neutral-500 hover:text-neutral-800 font-semibold text-sm py-3.5 transition-all rounded-lg mb-4 disabled:opacity-50"
            >
            ...
            <Link
              href="/"
              className="w-full flex items-center justify-center gap-2 text-neutral-400 hover:text-neutral-700 font-medium text-sm py-2 transition-all"
            >
  ...
  const handleSavePromo = (promo: PromoBanner) => {
    if (editingPromo) {
    ...
    showSuccess(editingPromo ? "Promotion mise à jour" : "Nouvelle promotion créée");
  };
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-800" style={{ fontSize: "16px" }}>
      ...
      <header className="sticky top-0 z-40 w-full border-b border-neutral-200 bg-white shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-8 py-5">
          <div className="flex items-center gap-5">
            ...
            <div>
              <span className="text-base font-bold tracking-[0.1em] text-neutral-900 uppercase block leading-none">Perfum Guy</span>
      ...
      <main className="mx-auto max-w-7xl px-8 py-8 space-y-8">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            { label: "Revenus totaux", value: `${Math.round(totalSales * 135).toLocaleString("fr-DZ")} DA`, icon: DollarSign },
                  ...
                  {orders.slice(0, 5).map(order => (
                    <div key={order.id} className="flex items-center justify-between border-b border-neutral-100 pb-4 last:border-0">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-bold text-neutral-900">{Math.round(order.totalPrice * 135).toLocaleString("fr-DZ")} DA</span>
                      ...
                      <tbody className="divide-y divide-neutral-100">
                        {filteredProducts.map(product => (
                          <tr key={product.id} className="hover:bg-neutral-50 transition-colors">
                            ...
                            <td className="px-5 py-4 text-right"><span className="text-sm font-bold text-neutral-900">{Math.round(product.price * 135).toLocaleString("fr-DZ")} DA</span></td>
                            <td className="px-5 py-4 text-center">
                              {(() => {
                                const total = product.stock["50ml"] + product.stock["100ml"];
                                const isOos = total === 0;
                                return (
                                  <button
                                    onClick={() => {
                                      if (isOos) {
                                        updateProductStock(product.id, "50ml", 10);
                                        updateProductStock(product.id, "100ml", 10);
                                        showSuccess("Stock rétabli");
                                      } else {
                                        updateProductStock(product.id, "50ml", 0);
                                        updateProductStock(product.id, "100ml", 0);
                                        showSuccess("Produit mis en rupture");
                                      }
                                    }}
                                    title={isOos ? "Rétablir le stock" : "Mettre en rupture"}
                                    className={`inline-flex items-center gap-1.5 text-[8px] font-black uppercase tracking-wider px-2.5 py-1.5 rounded-lg border transition-all ${
                                      isOos
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-0.5">
                          <span className="text-[8px] uppercase tracking-[0.15em] text-neutral-400 font-bold">{stat.label}</span>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="relative w-48">
                      ...
                      <input type="text" placeholder="Rechercher..." value={stockSearchTerm} onChange={(e) => setStockSearchTerm(e.target.value)}
                        className="w-full bg-white border border-neutral-200 pl-9 pr-4 py-2.5 text-[10px] text-neutral-800 placeholder-neutral-400 focus:outline-none focus:border-neutral-400" />
                    </div>
                    <select value={stockSortBy} onChange={(e) => setStockSortBy(e.target.value as SortBy)}
                      className="bg-white border border-neutral-200 px-3 py-2.5 text-[10px] text-neutral-700 focus:outline-none focus:border-neutral-400">
                    ...
                    <div className="flex items-center border border-neutral-200">
                      {([
                        ...
                        <button key={f.value} onClick={() => setStockFilter(f.value)}
                          className={`text-[8px] font-bold uppercase tracking-[0.1em] px-3 py-2 transition-all ${
                    ...
                    <button onClick={() => setShowLowStockOnly(!showLowStockOnly)}
                      className={`flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-[0.1em] px-3 py-2 border transition-all ${
                ...
                <div className="bg-white border border-neutral-200 overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b border-neutral-200 bg-neutral-50">
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400">Produit</th>
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400 text-center">50ml</th>
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400 text-center">100ml</th>
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400 text-center">Total</th>
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400 text-center">Statut</th>
                          <th className="p-3 text-[9px] font-bold uppercase tracking-[0.1em] text-neutral-400 text-center">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-neutral-100">
                            ...
                            <tr key={product.id} className="hover:bg-neutral-50/50 transition-colors">
                              <td className="p-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                    <p className="text-[10px] font-bold text-neutral-800 truncate">{product.name}</p>
                                    <p className="text-[8px] text-neutral-400">{product.category}</p>
                                  </div>
                                </div>
                              </td>
                              <td className="p-3 text-center">
                                {isEditing ? (
                                  <input type="number" min="0" value={editStockValues["50ml"]}
                                    onChange={(e) => setEditStockValues(v => ({ ...v, "50ml": parseInt(e.target.value) || 0 }))}
                                    className="w-16 border border-neutral-300 px-2 py-1 text-[10px] text-center focus:outline-none focus:border-neutral-500" />
                                ) : (
                                  <span className={`text-[10px] font-bold ${product.stock["50ml"] <= (product.lowStockAlert || 5) ? 'text-neutral-700' : 'text-neutral-700'}`}>
                                    {product.stock["50ml"]}
                                  </span>
                                )}
                              </td>
                              <td className="p-3 text-center">
                                {isEditing ? (
                                  <input type="number" min="0" value={editStockValues["100ml"]}
                                    onChange={(e) => setEditStockValues(v => ({ ...v, "100ml": parseInt(e.target.value) || 0 }))}
                                    className="w-16 border border-neutral-300 px-2 py-1 text-[10px] text-center focus:outline-none focus:border-neutral-500" />
                                ) : (
                                  <span className={`text-[10px] font-bold ${product.stock["100ml"] <= (product.lowStockAlert || 5) ? 'text-neutral-700' : 'text-neutral-700'}`}>
                                    {product.stock["100ml"]}
                                  </span>
                                )}
                              </td>
                              <td className="p-3 text-center">
                                <span className={`text-[10px] font-bold ${status === 'out-of-stock' ? 'text-red-500' : status === 'low-stock' ? 'text-neutral-700' : 'text-neutral-700'}`}>
                                  {totalStock}
                                </span>
                              </td>
                              <td className="p-3 text-center">
                                <span className={`inline-flex items-center gap-1 text-[8px] font-bold uppercase tracking-[0.08em] px-2 py-1 border ${
            ...
            {activeTab === "hero" && (
              <div className="space-y-6">
                ...
                <div className="bg-white border border-neutral-200 rounded-xl p-5">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.15em] text-neutral-400 mb-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
                      ...
                      {heroProductIds.length > 0 && (
                        <button
                          ...
                          className="text-[9px] text-red-400 hover:text-red-600 font-bold uppercase tracking-wider transition-colors"
                        >
                          Tout vider
                        </button>
                      )}
                    ...
                    {heroProductIds.length === 0 ? (
                      <div className="py-16 text-center">
                        ...
                        <p className="text-[10px] text-neutral-300 mt-1">Ajoutez des parfums depuis le catalogue ci-contre</p>
                      </div>
                    ) : (
                      <div className="divide-y divide-neutral-100">
                        {heroProductIds.map((id, idx) => {
                          ...
                          if (!p) return null;
                          const total = (p.stock?.["50ml"] || 0) + (p.stock?.["100ml"] || 0);
                          const isOos = total === 0;
                          ...
                          return (
                            <div key={id} className="flex items-center gap-3 px-4 py-3 hover:bg-neutral-50 transition-colors">
                              {/* Position badge */}
                              <span className="flex-shrink-0 w-6 h-6 bg-neutral-100 rounded-full flex items-center justify-center text-[9px] font-black text-neutral-500">
                                {idx + 1}
                              </span>
                              {/* Thumbnail */}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-[9px] text-neutral-400">{Math.round(p.price * 135).toLocaleString("fr-DZ")} DA</span>
                                  {hasPromo && <span className="text-[8px] font-black text-red-500 bg-red-50 px-1.5 py-0.5 rounded">-{p.discountPercent}%</span>}
                                  {isOos && <span className="text-[8px] font-black text-neutral-400 bg-neutral-100 px-1.5 py-0.5 rounded">Épuisé</span>}
                                </div>
                              </div>
                              {/* Controls */}
                  ...
                  <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-neutral-100">
                      ...
                      <p className="text-[10px] text-neutral-400 mt-0.5">Cliquez sur un parfum pour l'ajouter au carousel</p>
                    </div>
                      ...
                      {products.map(p => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Info */}
                            <div className="flex-1 min-w-0">
                              <p className="text-[11px] font-bold text-neutral-800 truncate">{p.name}</p>
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-[9px] text-neutral-400">{p.category}</span>
                                {hasPromo && <span className="text-[8px] font-black text-red-500">-{p.discountPercent}%</span>}
                                {isOos && <span className="text-[8px] text-neutral-400">Épuisé</span>}
                              </div>
                            </div>
                            {/* Status */}
                            <div className="flex-shrink-0">
                              {alreadyIn ? (
                                <span className="inline-flex items-center gap-1 text-[8px] font-black uppercase bg-neutral-900 text-white px-2 py-1 rounded-full">
                                  <CheckCircle2 className="h-2.5 w-2.5" /> Ajouté
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 text-[8px] font-black uppercase text-neutral-400 bg-neutral-100 px-2 py-1 rounded-full hover:bg-neutral-900 hover:text-white transition-colors">
            ...
            {activeTab === "sections" && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-4 bg-white border border-neutral-200 p-6 space-y-4">
                  ...
                  {catError && <p className="text-[9px] text-red-500">{catError}</p>}
                  <form onSubmit={handleAddCategory} className="space-y-3">
                    <div>
                      <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1.5">Titre</label>
                      <input type="text" placeholder="ex: Parfums Floraux" value={newCatName} onChange={(e) => setNewCatName(e.target.value)}
                        className="w-full border border-neutral-200 bg-white px-3 py-2.5 text-[10px] text-neutral-800 placeholder-neutral-300 focus:outline-none focus:border-neutral-400" required />
                    </div>
                    <div>
                      <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1.5">Description</label>
                      <textarea placeholder="Description courte..." value={newCatDesc} onChange={(e) => setNewCatDesc(e.target.value)} rows={3}
                        className="w-full border border-neutral-200 bg-white px-3 py-2.5 text-[10px] text-neutral-800 placeholder-neutral-300 focus:outline-none focus:border-neutral-400 resize-none" required />
                    </div>
                    <div>
                      <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1.5">Icône de Section</label>
                    ...
                    <div>
                      <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1.5">URL Image de Couverture (Optionnel)</label>
                      <input type="text" placeholder="https://images.unsplash.com/..." value={newCatImageUrl} onChange={(e) => setNewCatImageUrl(e.target.value)}
                        className="w-full border border-neutral-200 bg-white px-3 py-2.5 text-[10px] text-neutral-800 placeholder-neutral-300 focus:outline-none focus:border-neutral-400" />
                    </div>
                    <div className="flex gap-2 pt-2">
                      <button type="submit" className="flex-1 bg-neutral-900 hover:bg-black text-white font-medium uppercase tracking-[0.1em] text-[9px] py-3 transition-all">
                      ...
                      {editingCategory && (
                        <button type="button" onClick={() => {
                          ...
                          setNewCatImageUrl("");
                          setCatError("");
                        }} className="border border-neutral-200 text-neutral-500 font-medium uppercase tracking-[0.1em] text-[9px] px-4 py-3 transition-all hover:bg-neutral-50">
                          Annuler
                        </button>
                      )}
                    </div>
                        ...
                        <div key={cat.id} className="bg-white border border-neutral-200 p-4 space-y-3 flex flex-col justify-between">
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <h4 className="text-[11px] font-bold text-neutral-800 uppercase tracking-[0.08em]">{cat.name}</h4>
                            </div>
                            <p className="text-[9px] text-neutral-500 font-light leading-relaxed">{cat.description}</p>
                          </div>
                          <div className="flex justify-end gap-3 pt-2 border-t border-neutral-100 mt-2">
                            ...
                            }} className="flex items-center gap-1.5 text-[8px] uppercase tracking-[0.1em] text-neutral-400 hover:text-neutral-900 font-bold transition-colors">
                              <Edit2 className="h-3 w-3" /> Modifier
                            </button>
                            <button onClick={() => { deleteCategory(cat.id); showSuccess("Section supprimée"); }}
                              className="flex items-center gap-1.5 text-[8px] uppercase tracking-[0.1em] text-neutral-400 hover:text-red-500 font-bold transition-colors">
              ...
              <div className="space-y-3">
                ...
                {orders.length === 0 ? (
                  <div className="text-center py-16 bg-white border border-neutral-200">
                    ...
                    <p className="text-[10px] text-neutral-400">Aucune commande</p>
                  </div>
                ) : (
                  orders.map(order => (
                    <div key={order.id} className="bg-white border border-neutral-200 p-4 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-neutral-100">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                          <span className="text-[11px] font-bold text-neutral-800">{order.id}</span>
                          <span className="text-[10px] font-extrabold text-neutral-800">{order.firstName || ""} {order.lastName || ""}</span>
                          <span className="text-[10px] font-bold text-accent">{order.phone || ""}</span>
                          <span className="text-[9px] text-neutral-500 bg-neutral-100 px-2 py-0.5">📍 {order.residence || ""}, {order.wilaya || ""}</span>
                          {order.customerEmail && <span className="text-[9px] text-neutral-400 font-medium">({order.customerEmail})</span>}
                          <span className="text-[8px] text-neutral-400">{order.createdAt}</span>
                        </div>
                        <button onClick={() => handleOrderStatusToggle(order.id, order.status)}
                          className={`inline-flex items-center gap-1.5 text-[8px] uppercase tracking-[0.08em] px-2.5 py-1.5 border font-bold transition-all ${getStatusStyle(order.status)}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                        ...
                        <div className="text-right">
                          <span className="text-[8px] uppercase tracking-[0.1em] text-neutral-400 block">Total</span>
            ...
            {activeTab === "promo" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-serif text-sm tracking-[0.1em] text-neutral-700 uppercase">Promotions ({promos.length})</h3>
                  <button onClick={() => { setEditingPromo(null); setShowPromoForm(true); }}
                    className="flex items-center gap-2 bg-neutral-900 hover:bg-black text-white font-medium uppercase tracking-[0.1em] text-[9px] px-4 py-2.5 transition-all">
                    <Plus className="h-3.5 w-3.5" /> Nouvelle Promo
                  </button>
                </div>
                <AnimatePresence>
                  {showPromoForm && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
                        className="bg-white border border-neutral-200 p-6 w-full max-w-lg space-y-4 shadow-lg"
                        onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between">
                          <h4 className="font-serif text-sm text-neutral-800 uppercase tracking-[0.1em]">{editingPromo ? "Modifier" : "Nouvelle"}</h4>
                          <button onClick={() => setShowPromoForm(false)} className="text-neutral-400 hover:text-neutral-700"><X className="h-4 w-4" /></button>
                        </div>
                        <PromoForm initial={editingPromo || undefined} onSave={handleSavePromo} onCancel={() => setShowPromoForm(false)} />
                      </motion.div>
                    </motion.div>
                  )}
                </AnimatePresence>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  {promos.map(promo => (
                    <div key={promo.id} className="bg-white border border-neutral-200 p-4 space-y-3">
                      <div className="flex items-start gap-3">
                        <div className="h-20 w-16 overflow-hidden border border-neutral-200 flex-shrink-0">
                          <img src={promo.image} alt={promo.title} className="h-full w-full object-cover" />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-rose-50 text-rose-600 text-[8px] font-bold px-2 py-0.5">-{promo.discountPercent}%</span>
                            <span className={`text-[8px] font-bold uppercase ${promo.active ? 'text-emerald-600' : 'text-neutral-400'}`}>{promo.active ? 'Active' : 'Inactive'}</span>
                          </div>
                          <h4 className="text-[11px] font-bold text-neutral-800">{promo.title}</h4>
                          <p className="text-[9px] text-neutral-500 mt-0.5">{promo.description}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className="text-[9px] text-neutral-400 line-through">${promo.originalPrice}</span>
                            <span className="text-[10px] font-bold text-neutral-900">${(promo.originalPrice * (1 - promo.discountPercent / 100)).toFixed(0)}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex justify-end gap-2 pt-2 border-t border-neutral-100">
                        <button onClick={() => { setEditingPromo(promo); setShowPromoForm(true); }}
                          className="text-[8px] uppercase tracking-[0.08em] text-neutral-500 hover:text-neutral-800 font-bold"><Edit2 className="h-3 w-3 inline mr-1" />Modifier</button>
                        <button onClick={() => handleDeletePromo(promo.id)}
                          className="text-[8px] uppercase tracking-[0.08em] text-neutral-400 hover:text-red-500 font-bold"><Trash2 className="h-3 w-3 inline mr-1" />Supprimer</button>
                        <button onClick={() => { setPromos(prev => prev.map(p => p.id === promo.id ? { ...p, active: !p.active } : p)); showSuccess(`Promotion ${promo.active ? 'désactivée' : 'activée'}`); }}
                          className={`text-[8px] uppercase tracking-[0.08em] font-bold px-2 py-0.5 border ${promo.active ? 'border-red-300 text-red-500' : 'border-emerald-300 text-emerald-600'}`}>{promo.active ? 'Désactiver' : 'Activer'}</button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="space-y-2 max-h-80 overflow-y-auto">
                    {products.filter(p => !nouveautes.some(n => n.productId === p.id)).map(product => (
                      <button key={product.id} onClick={() => handleAddNouveaute(product.id)}
                        ...
                        <div className="min-w-0">
                          <p className="text-[10px] font-medium text-neutral-700 truncate">{product.name}</p>
                          <p className="text-[8px] text-neutral-400">${product.price}</p>
                        </div>
                  ...
                  {nouveautes.map(nouveaute => {
                    ...
                    return (
                      <div key={nouveaute.id} className="bg-white border border-neutral-200 p-4 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] font-bold text-neutral-800">{product.name}</p>
                          <p className="text-[8px] text-neutral-400">{product.category} • ${product.price}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <input type="text" placeholder="Badge" value={nouveaute.badge || ""} onChange={(e) => handleNouveauteBadge(nouveaute.id, e.target.value)}
                              className="border border-neutral-200 px-2 py-1 text-[8px] text-neutral-700 w-28 focus:outline-none focus:border-neutral-400" />
                            <button onClick={() => handleNouveauteToggle(nouveaute.id)}
                              className={`text-[8px] font-bold uppercase px-2 py-1 border ${nouveaute.featured ? 'border-neutral-300 text-neutral-700 bg-neutral-50' : 'border-neutral-200 text-neutral-400'}`}>
                              {nouveaute.featured ? '★' : '☆'}
                            </button>
                          </div>
                        </div>
              ...
              <div className="space-y-8">
                ...
                <div className="bg-white border border-neutral-200 rounded-xl p-6 space-y-4">
                  ...
                  <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-4 space-y-3">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">{editingBrand ? `Modifier : ${editingBrand.name}` : "Ajouter une marque"}</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                    {brands.map((brand: Brand) => (
                      <div key={brand.id} className="group relative bg-neutral-50 border border-neutral-200 rounded-xl p-3 flex flex-col items-center gap-2">
                        ...
                        <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider text-center truncate w-full">{brand.name}</p>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {marques.map(marque => (
                    <div key={marque.id} className="bg-white border border-neutral-200 p-5 text-center space-y-3 group">
                      ...
                      <div>
                        <h4 className="text-[10px] font-bold text-neutral-800 uppercase tracking-wider">{marque.logoType === "text" ? marque.logo : marque.name}</h4>
                        <p className="text-[8px] text-neutral-400 mt-0.5">{marque.description}</p>
                      </div>
...
function PromoForm({ initial, onSave, onCancel }: {
  initial?: PromoBanner;
  onSave: (promo: PromoBanner) => void;
  onCancel: () => void;
}) {
  const [form, setForm] = useState({
    id: initial?.id || "", title: initial?.title || "", description: initial?.description || "",
    discountPercent: initial?.discountPercent || 20, originalPrice: initial?.originalPrice || 100,
    image: initial?.image || "", active: initial?.active ?? true,
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="space-y-3">
      <div>
        <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Titre</label>
      ...
      <div>
        ...
        <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={2}
          className="w-full border border-neutral-200 px-3 py-2.5 text-[10px] text-neutral-800 focus:outline-none focus:border-neutral-400 resize-none" required />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Remise (%)</label>
          <input type="number" value={form.discountPercent} onChange={e => setForm(p => ({ ...p, discountPercent: parseInt(e.target.value) }))}
            className="w-full border border-neutral-200 px-3 py-2.5 text-[10px] text-neutral-800 focus:outline-none focus:border-neutral-400" required />
        </div>
        <div>
          <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Prix Original</label>
          <input type="number" value={form.originalPrice} onChange={e => setForm(p => ({ ...p, originalPrice: parseInt(e.target.value) }))}
            className="w-full border border-neutral-200 px-3 py-2.5 text-[10px] text-neutral-800 focus:outline-none focus:border-neutral-400" required />
        </div>
      </div>
      <div>
        <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Image URL</label>
...
function MarqueForm({ initial, onSave, onCancel, onLogoUpload }: {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="space-y-3">
      ...
      <div>
        <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Logo</label>
        <div className="flex gap-2 mb-2">
          <button type="button" onClick={() => setForm(p => ({ ...p, logoType: "text" }))}
            className={`text-[8px] font-bold uppercase px-2 py-1 border ${form.logoType === "text" ? "border-neutral-400 bg-neutral-100 text-neutral-800" : "border-neutral-200 text-neutral-400"}`}>Texte</button>
          <button type="button" onClick={() => setForm(p => ({ ...p, logoType: "image" }))}
            className={`text-[8px] font-bold uppercase px-2 py-1 border ${form.logoType === "image" ? "border-neutral-400 bg-neutral-100 text-neutral-800" : "border-neutral-200 text-neutral-400"}`}>Image</button>
        </div>
        {form.logoType === "text" ? (
          <input type="text" value={form.logo} onChange={e => setForm(p => ({ ...p, logo: e.target.value }))}
            placeholder="Ex: CHANEL"
            className="w-full border border-neutral-200 px-3 py-2.5 text-[10px] text-neutral-800 placeholder-neutral-300 focus:outline-none focus:border-neutral-400" required />
        ) : (
          <div className="space-y-2">
            {form.logo && form.logoType === "image" ? (
              ...
              <button type="button" onClick={() => fileInputRef.current?.click()}
                className="flex items-center gap-2 border border-dashed border-neutral-300 p-4 text-[10px] text-neutral-400 hover:text-neutral-600 hover:border-neutral-400 transition-all">
      ...
      <div>
        <label className="block text-[8px] font-bold uppercase tracking-[0.1em] text-neutral-400 mb-1">Site Web (optionnel)</label>
